...
class MessagesController {
  ...
  async connect(id: number) {
    ...
    const { token } = await ChatController.fetchToken({ id })
    ...
    switch (type) {
      ...
      case 'messages': {
        if (oldMessages.length) {
          const idOfOldMsgs = (oldMessages as IMessage[]).reduce(
            (accum: number[], msg: IMessage) => {
              accum.push(msg.id)
              return accum
            },
            [],
          ...
          message = (message as IMessage[]).filter(
